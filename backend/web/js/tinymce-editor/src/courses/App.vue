<template>
	<div class="app">
		...
		<TinyMCEContent v-model="pageData.desc"/>
		<AttachmentFieldCard
			:files="remote( 'data/images' )"
			:url-prefix="remote( 'data/publicUrl' )"
			attachment-key="file_id"
...
export default {
	...
	methods: {
		deleteAttachment() {
			this.$set( this.pageData, 'file_id', 0 );
		},
		setAttachment( selected ) {
			this.$set( this.pageData, 'file_id', +selected.id );
		},
		savePage() {
			this.ajax( {
				...this.remote( 'request' ),
